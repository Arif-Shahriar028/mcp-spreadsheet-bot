export function extractStructuredData(values: string[][]) {
  const individualBazarSummaryInTaka: Record<string, string> = {};
  const expenseDetails: { date: string; name: string; price: string }[] = [];
  const mealTracker: any[] = [];
